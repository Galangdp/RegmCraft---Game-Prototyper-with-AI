...
status_t command_uinit(matroot_t *matroot, string_t *error_message, uint8_t palette_id, json_t *out) {
    ...
    json_array_t *persons_jarr = json_object_cset_empty_array(out_jobj, "persons");
    ...
    palette_t *current_palette = matroot->palettes.entries + palette_id;
    ...
    for (size_t i = 0; i < matroot->models.length; i++) {
        ...
        switch (model->type) {
            case MODEL_TYPE_PERSON:
                model_jobj = json_array_push_empty_object(persons_jarr);
                break;
    ...
    json_object_cset_int64(out_jobj, "paletteId", palette_id);
    ...
    if (image_encode(&base_pixels, buffer, &bound) != EXIT_SUCCESS) {
    ...
    if (file_open(&base_file, COMMAND_BASE_PATH, FILE_OMODE_WR) != EXIT_SUCCESS) {
    ...
    if (file_write_byte(&base_file, buffer, bound) != EXIT_SUCCESS) {
