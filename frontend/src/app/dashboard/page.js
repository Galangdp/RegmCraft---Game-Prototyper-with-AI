...
export default function DashboardPage() {
  ...
  const filteredProjects = useMemo(() => {
    ...
    let filtered = projects.filter((project) => {
      ...
      if (activeTab === "trash" && !project.isDeleted) return false;
      if (activeTab !== "trash" && project.isDeleted) return false;

      // Filter by Recent (last 24 hours) - maintained for the "Recent" tab view
      if (activeTab === "recent") {
    ...
    // Differentiated Sorting Logic
    if (activeTab === "all") {
      // stable sort by creation date
      filtered.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
    } else {
      // sort by last modified for recent and trash
      filtered.sort((a, b) => new Date(b.lastModified) - new Date(a.lastModified));
    }

    // Visual Hint: Limit Recent in the dashboard tab to 6
    if (activeTab === "recent") {
      ...
      <div className="flex h-screen items-center justify-center bg-neutral-950 text-white font-sans">
        <div className="flex flex-col items-center space-y-4">
          ...
          <p className="text-neutral-500 font-medium tracking-tight">
            {!user && !authLoading ? "Redirecting to Login..." : "Loading RegmCraft Workspace..."}
          </p>
        </div>
      </div>
    );
          ...
          <header className="mb-10">
            <h1 className="text-3xl font-bold flex items-center space-x-3 tracking-tight">
              <span>
                ...
                {activeTab === "recent" && "Recent Drafts"}
          ...
          {filteredProjects.length === 0 ? (
            ...
            >
              ...
              <h3 className="text-xl font-semibold text-neutral-400">
                {searchQuery ? "No projects match your search" : "No projects found here"}
              </h3>
              <p className="text-neutral-500 mt-2 max-w-xs px-4">
                {activeTab === "trash"
                  ? "Items in trash will be permanently deleted soon."
                  : "Start by creating a new draft project using the AI assistant."}
              </p>
              {!searchQuery && activeTab !== "trash" && (
                <button
                  onClick={() => setIsModalOpen(true)}
                  className="mt-8 px-8 py-3 bg-blue-600 hover:bg-blue-500 rounded-xl text-sm font-bold transition-all shadow-lg shadow-blue-600/20 active:scale-95"
                >
